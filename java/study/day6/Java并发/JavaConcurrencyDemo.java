...
public class JavaConcurrencyDemo {
    ...
    //  Q2.5: 多核和多线程的关系是什么？
    // ================================================================
    static void Q2_5_MultiCoreVsMultithread() {
        printH2("Q2.5: 多核和多线程的关系是什么？");
        log("");
        log("  多核 = 硬件（CPU 有几个物理核心）");
        log("  多线程 = 软件（程序开了几个执行路径）");
        log("");
        log("  关系：多线程是手段，多核是舞台——多线程的终极目的是跑满多核。");
        log("");
        log("  ┌──────────────────────────────────────────────────────────┐");
        log("  │  单核 CPU 上跑多线程：                                    │");
        log("  │    CPU 一个核，快速在多个线程间切换（时间片轮转）。       │");
        log("  │    看起来「同时」，其实是「高速交替」。                   │");
        log("  │    上下文切换还有额外开销，线程太多反而更慢！             │");
        log("  ├──────────────────────────────────────────────────────────┤");
        log("  │  多核 CPU 上跑多线程：                                    │");
        log("  │    每个核可以独立运行一个线程 → 真正并行！                │");
        log("  │    4 核 = 最多同时跑 4 个线程（其他在排队等核）。        │");
        log("  │    这就是为什么线程数 = 核数时吞吐量最大。               │");
        log("  └──────────────────────────────────────────────────────────┘");
        log("");
        log("  比喻：");
        log("    单核 = 一个收银台，多个顾客轮着用（并发，concurrent）");
        log("    多核 = 多个收银台，每个台一个顾客（并行，parallel）");
        log("");
        log("  关键认知：");
        log("    「并发」≠「并行」！");
        log("    并发：一段时间内多个任务交替执行（单核也能并发）");
        log("    并行：同一时刻多个任务真正同时执行（必须多核）");
        log("");
        log("  线程数 > 核数 → 多出来的线程分时复用 CPU，产生上下文切换开销。");
        log("  CPU 密集型任务 → 线程数 ≈ 核数 + 1");
        log("  IO 密集型任务  → 线程数可远大于核数（因为大部分时间在等 IO）");
    }
    ...
    static void Q5_OptimisticPessimisticLock() {
        ...
        log("");
        log("  ─── CAS 中的「自旋」是什么？ ───");
        log("    自旋 = CAS 失败后不阻塞，原地循环重试。");
        log("");
        log("    代码本质上就是：");
        log("      while (!atomic.compareAndSet(old, new)) {");
        log("          old = atomic.get();  // 重新读最新值");
        log("      }");
        log("");
        log("    比喻：就像自动售货机没出货 → 你狂按按钮直到出来为止。");
        log("    没拿到锁也不去睡觉(阻塞)，原地猛试(自旋)。");
        log("");
        log("    为什么自旋不阻塞？");
        log("      阻塞 = 线程放弃 CPU，让操作系统切走 → 上下文切换开销很大");
        log("      自旋 = 线程不放弃 CPU，原地循环等 → 省了切换开销");
        log("      → 适合锁持有时间极短的场景（别人马上就释放了）");
        log("      → 不适合锁持有长的场景（自旋白白吃 CPU）");
        log("");
        log("    JVM 的「自适应自旋锁」：");
        log("      如果上次自旋成功了，这次就多自旋几次；");
        log("      如果上次失败了，这次就少自旋或直接阻塞。");
        log("      → 智能判断，不用你手动指定。");
        ...
        while (!counter.compareAndSet(0, 1)) {
            spinCount++;  // 实际不会进来，演示写法
        }
        log("    实测: 自旋了 " + spinCount + " 次就成功了（首次就成功，理想情况）");
        log("");
        log("  ─── 自旋 vs 阻塞对比 ───");
        log("  ┌──────────────┬────────────────┬──────────────────┐");
        log("  │              │ 自旋(CAS重试)  │ 阻塞(synchronized)│");
        log("  ├──────────────┼────────────────┼──────────────────┤");
        log("  │ CPU 状态     │ 占着不放       │ 放弃让给别人     │");
        log("  │ 上下文切换   │ 无（0 开销）   │ 有（大开销）     │");
        log("  │ 适合场景     │ 锁持有时间短    │ 锁持有时间长     │");
        log("  │ 缺点         │ 空转浪费 CPU    │ 切换开销大       │");
        log("  │ 经典用法     │ AtomicInteger  │ synchronized     │");
        log("  └──────────────┴────────────────┴──────────────────┘");
    }
